...
def users(request):
    dict_about = {"users": Account.objects.all()}
        ...
        if status == "Delivered":
            try:
                payment = Payment.objects.get(
                    payment_id=order.order_number, status=False
                )
                print(payment)
    ...
    if request.method == "POST":
        ...
        orders = (
            ...
            .values(
                ...
                "user_order_page__product__stock",
                total=Sum("order_total"),
            )
            .annotate(dcount=Sum("user_order_page__quantity"))
            .order_by("-total")
        )
    else:
        orders = (
            Order.objects.filter(
                created_at__year=year, created_at__month=month, payment__status=True
            )
            .values(
                "user_order_page__product__product_name",
                "user_order_page__product__stock",
...
def sales_report_month(request, id):
    ...
    order_count_in_month = Order.objects.filter(
        created_at__year=year, created_at__month=month, is_ordered=True
    ).count()
    orders = (
        Order.objects.filter(created_at__month=id, payment__status=True)
        .values(
            "user_order_page__product__product_name",
            "user_order_page__product__stock",
            total=Sum("order_total"),
    ...
    today_date = str(date.today())
    returned = (
        Order.objects.filter(created_at__month=id, is_returned=True)
        .values(
            "user_order_page__product__product_name",
            "user_order_page__product__stock",
            total=Sum("order_total"),
...
def sales_report_year(request, id):
    orders = (
        Order.objects.filter(created_at__year=id, payment__status=True)
        .values(
            "user_order_page__product__product_name",
            "user_order_page__product__stock",
            total=Sum("order_total"),
...
def pdf_report(request, start_date, end_date):
    ...
    response["Content-Disposition"] = (
        "attachment; filename=sales_report" + str(datetime.now()) + ".pdf"
    )
...
def excel_report(request, start_date, end_date):
    ...
    response["Content-Disposition"] = (
        "attachment; filename=sales_report" + str(datetime.now()) + ".xls"
    )
...
def admindash(request):
    ...
    order_count_in_day = Order.objects.filter(
        created_at__date=today, is_ordered=True
    ).count()
    order_count_in_week = Order.objects.filter(
        created_at__gte=one_week, is_ordered=True
    ).count()
    ...
    razorpay_orders = Payment.objects.filter(
        payment_method="RazerPay", status=True
    ).count()
    cash_on_delivery_count = Payment.objects.filter(
        payment_method="Cash On Delivery", status=True
    ).count()

    total_payment_count = paypal_orders + razorpay_orders + cash_on_delivery_count
    try:
        total_payment_amount = (
            Payment.objects.filter(status=True)
            .annotate(total_amount=Cast("amount_paid", FloatField()))
    ...
    if (
        isinstance(total_payment_amount, dict)
        and "total_amount__sum" in total_payment_amount
    ):
    ...
    today_sale = Order.objects.filter(
        created_at__date=today_date, payment__status=True, is_ordered=True
    ).count()
    today = today.strftime("%A")
    new_date = datetime.today() - timedelta(days=1)
    yester_day_sale = Order.objects.filter(
        created_at__date=new_date, payment__status=True, is_ordered=True
    ).count()
    yesterday = new_date.strftime("%A")
    new_date = new_date - timedelta(days=1)
    day_2 = Order.objects.filter(
        created_at__date=new_date, payment__status=True, is_ordered=True
    ).count()
    day_2_name = new_date.strftime("%A")
    new_date = new_date - timedelta(days=1)
    day_3 = Order.objects.filter(
        created_at__date=new_date, payment__status=True, is_ordered=True
    ).count()
    day_3_name = new_date.strftime("%A")
    new_date = new_date - timedelta(days=1)
    day_4 = Order.objects.filter(
        created_at__date=new_date, payment__status=True, is_ordered=True
    ).count()
    day_4_name = new_date.strftime("%A")
    new_date = new_date - timedelta(days=1)
    day_5 = Order.objects.filter(
        created_at__date=new_date, payment__status=True, is_ordered=True
    ).count()
