...
def cart(request, total=0, quantity=0, cart_items=None):
    ...
    try:
        ...
        for cart_item in cart_items:
            ...
            total += int(cart_item.product.offer_price()) * int(
                cart_item.quantity
            )  # *price_mult
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if float(grand_total) >= float(instance.coupon.min_value):
                coupon_discount = (
                    float(grand_total) * float(instance.coupon.discount)
                ) / 100
                ...
                msg = (
                    "This coupon is only applicable for orders more than ₹"
                    + str(instance.coupon.min_value)
                    + "\- only!"
                )
        except:
