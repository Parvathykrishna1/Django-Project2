    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user.is_active:
                ...
                otp = totp.now()
                msg_html = render_to_string("accounts/email.html", {"otp": otp})

                send_mail(
                    f"Please verify your E-mail",
                    f"Your One-Time Verification Password is {otp}",
...
def change_password(request):
    ...
    return render(request, "accounts/changepswd.html", {"form": form})
...
def verify_otp(request):
    ...
    if request.method == "POST":
        ...
        if user_otp != "":
            ...
            if "otp" in request.session and int(user_otp) == int(
                request.session["otp"]
            ):
                ...
                messages.success(request, "Email, please login to continue")
                return redirect("login")
            else:
